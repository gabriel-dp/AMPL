...
subject to max_espec_1_SA: x[1, "SA"] / (sum {p in PETROLEO} x[p, "SA"]) <= 0.3;
subject to min_espec_2_SA: x[2, "SA"] / (sum {p in PETROLEO} x[p, "SA"]) >= 0.4;
subject to max_espec_3_SA: x[3, "SA"] / (sum {p in PETROLEO} x[p, "SA"]) <= 0.5;

subject to max_espec_1_AZ: x[1, "AZ"] / (sum {p in PETROLEO} x[p, "AZ"]) <= 0.3;
subject to min_espec_2_AZ: x[2, "AZ"] / (sum {p in PETROLEO} x[p, "AZ"]) >= 0.1;

subject to max_espec_1_AM: x[1, "AM"] / (sum {p in PETROLEO} x[p, "AM"]) <= 0.7;